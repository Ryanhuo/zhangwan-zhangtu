...
| `design-review/` | 用户要求设计 Review、规范合规检查、核对页面是否对齐 zhangwan-design、识别硬编码色值/偏离令牌时 |
| `work-summary/` | 用户要求写日报/周报/工作总结，或梳理某时间段内页面/需求/迭代变更时 |
| `project-guide/` | 新人上手、意图不清、或需要掌图全流程/项目状态引导时 |
| `project-memory/` | 用户要求整理项目文档、沉淀 spec/需求索引、维护项目记忆与 memory-log 时 |
| `default-resource-recommendations/` | 用户询问默认图表/图标/字体/动画资源选型时（已对齐 zhangwan-design，不引入冲突资源） |
