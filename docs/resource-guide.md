...
- 当前仓库已预留 `src/resources/` 作为长期资料目录。
- 新建项目文档时，若内容不是页面私有事实源，默认保存到 `src/resources/`。
- 页面级产品事实源仍优先保留在各页面目录内的 `spec.md`。
- 若后续引入 `src/prototypes/`、`src/themes/` 等目录，继续按本指南的目录边界执行，不要求回迁已有页面私有素材。
